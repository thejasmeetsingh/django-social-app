"""
Contain string constants which will be used throughout the project
"""

INVALID_LOGIN = "Invalid login credentials"
LOGIN_SUCCESS = "Logged in successfully"
EMAIL_EXISTS = "User with this email already exists"
SIGNUP_SUCCESS = "Account created successfully"
INVALID_REQUEST_DATA = "Invalid request data passed"

USER_ID_ERROR = "Invalid user ID passed"
USER_NOT_EXISTS = "User with the given ID does not exists"
REQUEST_SUCCESS = "Request sent successfully"
REQUEST_STATUS_ERROR = "Invalid status passed"
REQUEST_UPDATE_ERROR = "You cannot update the request status"
REQUEST_UPDATE_SUCCESS = "Request status updated successfully"
REQUEST_LIMIT_ERROR = "You cannot send more than 3 request within a minute. Please try again after a minute."
